...
I've been in 💖 with Behat for four years now, and when [@mathieunapoli](http://twitter.com/mathieunapoli) asked me about how to setup Behat3 in one of his project I realized that I'm using some tricks in my scenarios that could be useful to anyone, so I might as well share them… I hope you'll find one or two useful!
...
The goal here is to go fast writing your tests, the quickest you go and the most time you can spend on production code. This is why you will find shortcuts, magic and a BaseContext class that have way too much responsabilities. It does not mean that it's awful code in the end because context classes are more simple than production code and I ensure you that I'm not pulling my hair out when I work on Behat code.
...
Everytime you need to use a method of Mink API, you generally either need to call it on the Session with `$this->getSession()->foo()` or on the Page object like `$this->getSession()->getPage()->foo()`. As we mostly will use these functions, and we are still not in production code, we could have some magic to our `BaseContext`:
...
Everyone who tried Behat with Mink and a JavaScript driver (I use Selenium2 with phantomjs) has had issues with trying to assert something in the current webpage while some JavaScript code has not been finished (pending Ajax query for example).

The [proper and recommended](http://docs.behat.org/en/v2.5/cookbook/using_spin_functions.html) way of dealing with theses issues is to use a `spin` method in your context, that will run the assertion or code multiple time before failing. Here is my implementation that you can add to your `BaseContext`:
...
If you're trying to assert something that should occur on a webpage, use Mink's `ExpectationException` rather than another exception. If thrown Behat will print the HTML code of the page if run with the verbose option, it will also open the HTML in your browser if you properly setted your `behat.yml` configuration:
...
One easily forgotten feature of Behat is that you can write several step definitions for a single step code, this is usefull to be syntaxically correct when dealing with numbers for example:
